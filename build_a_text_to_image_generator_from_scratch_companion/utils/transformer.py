...
# %% auto #0
__all__ = ['DEFAULT_DEVICE', 'DEVICE', 'OptimizerSchedule', 'subsequent_mask', 'make_std_mask', 'Batch', 'Transformer', 'Encoder',
           'EncoderLayer', 'SublayerConnection', 'LayerNorm', 'Decoder', 'DecoderLayer', 'Embeddings',
           'PositionalEncoding', 'attention', 'MultiHeadedAttention', 'Generator', 'PositionwiseFeedForward',
           'create_model', 'LabelSmoothing', 'SimpleLossCompute', 'NoamOpt']
...
def subsequent_mask(size: int) -> Tensor:
    ...
    future_positions: Tensor = torch.triu(
        torch.ones((1, size, size), dtype=torch.bool), diagonal=1
    )
...
class EncoderLayer(nn.Module):
    ...
    def forward(self, inputs: Tensor, mask: Tensor) -> Tensor:
        ...
        attended: Tensor = self.sublayer[0](
            inputs, lambda normalized: self.self_attn(
...
class SublayerConnection(nn.Module):
    ...
    def forward(
        self, inputs: Tensor, sublayer: Callable[[Tensor], Tensor]
    ) -> Tensor:
...
class DecoderLayer(nn.Module):
    ...
    def forward(
        ...
        decoded: Tensor = self.sublayer[0](
            inputs,
            lambda normalized: self.self_attn(
                normalized, normalized, normalized, tgt_mask
            ),
        )
        decoded = self.sublayer[1](
            decoded,
            lambda normalized: self.src_attn(
                normalized, memory, memory, src_mask
            ),
        )
...
class PositionalEncoding(nn.Module):
    ...
    def __init__(
        ...
        positional_encoding: Tensor = torch.zeros(
            max_len, d_model, device=device
        )
...
class MultiHeadedAttention(nn.Module):
    ...
    def __init__(self, h: int, d_model: int, dropout: float = 0.1) -> None:
        ...
        self.linears = nn.ModuleList(
            [deepcopy(nn.Linear(d_model, d_model)) for _ in range(4)]
        )
...
class LabelSmoothing(nn.Module):
    ...
    def __init__(
        self, size: int, padding_idx: int, smoothing: float = 0.1
    ) -> None:
