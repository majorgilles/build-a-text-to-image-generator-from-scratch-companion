...
def subsequent_mask(size: int) -> Tensor:
    ...
    """
    # Shape: (1, size, size); True above the diagonal marks future positions to hide.
...
def make_std_mask(target: Tensor, padding_id: int) -> Tensor:
    ...
    # Both broadcast to (batch_size, sequence_length, sequence_length).
...
class Batch:
    ...
    def __init__(
        ...
        # (batch_size, source_sequence_length) -> (batch_size, 1, source_sequence_length)
        ...
        if trg is not None:
            ...
            # Decoder input and next-token target: each (batch_size, target_sequence_length - 1).
            self.trg = target[:, :-1]
            self.trg_y = target[:, 1:]
            # Shape: (batch_size, target_sequence_length - 1, target_sequence_length - 1)
            self.trg_mask = make_std_mask(self.trg, pad)
            # Scalar count of non-padding target tokens, used to normalize the loss.
...
class Transformer(nn.Module):
    ...
    def encode(self, src: Tensor, src_mask: Tensor) -> Tensor:
        ...
        Args:
            src: Token IDs shaped ``(batch_size, source_sequence_length)``.
            src_mask: Visibility mask shaped ``(batch_size, 1, source_sequence_length)``.

        Returns:
            Encoder memory shaped ``(batch_size, source_sequence_length, d_model)``.
        """
        # (batch_size, source_sequence_length) -> (batch_size, source_sequence_length, d_model)
    ...
    def decode(
        ...
        Args:
            memory: Encoder output shaped ``(batch_size, source_sequence_length, d_model)``.
            src_mask: Source mask shaped ``(batch_size, 1, source_sequence_length)``.
            tgt: Target token IDs shaped ``(batch_size, target_sequence_length)``.
            tgt_mask: Causal-and-padding mask shaped
                ``(batch_size, target_sequence_length, target_sequence_length)``.

        Returns:
            Decoder states shaped ``(batch_size, target_sequence_length, d_model)``.
        """
        # (batch_size, target_sequence_length) -> (batch_size, target_sequence_length, d_model)
    ...
    def forward(
        ...
        Args:
            src: Source token IDs shaped ``(batch_size, source_sequence_length)``.
            tgt: Target token IDs shaped ``(batch_size, target_sequence_length)``.
            src_mask: Source mask shaped ``(batch_size, 1, source_sequence_length)``.
            tgt_mask: Target mask shaped
                ``(batch_size, target_sequence_length, target_sequence_length)``.

        Returns:
            Decoder states shaped ``(batch_size, target_sequence_length, d_model)``;
            apply :class:`Generator` to obtain vocabulary log-probabilities.
        """
        # Shape: (batch_size, source_sequence_length, d_model)
...
class Encoder(nn.Module):
    ...
    def forward(self, inputs: Tensor, mask: Tensor) -> Tensor:
        ...
        Args:
            inputs: Embeddings shaped ``(batch_size, sequence_length, d_model)``.
            mask: Visibility mask shaped ``(batch_size, 1, sequence_length)``.

        Returns:
            Normalized encoder states shaped ``(batch_size, sequence_length, d_model)``.
        """
        # Shape stays (batch_size, sequence_length, d_model) through every layer.
...
class EncoderLayer(nn.Module):
    ...
    def forward(self, inputs: Tensor, mask: Tensor) -> Tensor:
        ...
        Args:
            inputs: States shaped ``(batch_size, sequence_length, d_model)``.
            mask: Visibility mask shaped ``(batch_size, 1, sequence_length)``.

        Returns:
            Updated states shaped ``(batch_size, sequence_length, d_model)``.
        """
...
class SublayerConnection(nn.Module):
    ...
    def forward(self, inputs: Tensor, sublayer: Callable[[Tensor], Tensor]) -> Tensor:
        ...
        Args:
            inputs: States shaped ``(batch_size, sequence_length, d_model)``.
            sublayer: Shape-preserving transformation of the normalized inputs.

        Returns:
            Residual sum shaped ``(batch_size, sequence_length, d_model)``.
        """
        # The sublayer preserves (batch_size, sequence_length, d_model), so the residual adds directly.
...
class LayerNorm(nn.Module):
    ...
    def forward(self, inputs: Tensor) -> Tensor:
        ...
        Args:
            inputs: Any tensor shaped ``(..., features)``.

        Returns:
            Normalized tensor with the same shape as ``inputs``.
        """
        # Statistics keep a trailing singleton, (..., features) -> (..., 1), to broadcast back.
...
class Decoder(nn.Module):
    ...
    def forward(
        ...
        Args:
            inputs: Target embeddings shaped ``(batch_size, target_sequence_length, d_model)``.
            memory: Encoder output shaped ``(batch_size, source_sequence_length, d_model)``.
            src_mask: Source mask shaped ``(batch_size, 1, source_sequence_length)``.
            tgt_mask: Target mask shaped
                ``(batch_size, target_sequence_length, target_sequence_length)``.
        ...
        Returns:
            Normalized decoder states shaped ``(batch_size, target_sequence_length, d_model)``.
        """
        # Shape stays (batch_size, target_sequence_length, d_model) through every layer.
...
class DecoderLayer(nn.Module):
    ...
    def forward(
        ...
        Args:
            inputs: Target states shaped ``(batch_size, target_sequence_length, d_model)``.
            memory: Encoder output shaped ``(batch_size, source_sequence_length, d_model)``.
            src_mask: Source mask shaped ``(batch_size, 1, source_sequence_length)``.
            tgt_mask: Target mask shaped
                ``(batch_size, target_sequence_length, target_sequence_length)``.
        ...
        Returns:
            Updated target states shaped ``(batch_size, target_sequence_length, d_model)``.
        """
        ...
        # Cross-attention: queries span target_sequence_length, keys/values span
        # source_sequence_length from memory (batch_size, source_sequence_length, d_model).
...
class Embeddings(nn.Module):
    ...
    def forward(self, token_ids: Tensor) -> Tensor:
        ...
        Args:
            token_ids: Integer IDs shaped ``(batch_size, sequence_length)``.

        Returns:
            Embeddings shaped ``(batch_size, sequence_length, d_model)``.
        """
...
class PositionalEncoding(nn.Module):
    ...
    def __init__(
        ...
        # (max_len, 1) * (d_model / 2,) broadcasts to (max_len, d_model / 2).
        ...
        # Store as (1, max_len, d_model) so it broadcasts over the batch dimension.
        self.register_buffer("pe", positional_encoding.unsqueeze(0))

    def forward(self, inputs: Tensor) -> Tensor:
        """Add fixed position vectors to token embeddings.

        Args:
            inputs: Embeddings shaped ``(batch_size, sequence_length, d_model)``.

        Returns:
            Position-aware embeddings shaped ``(batch_size, sequence_length, d_model)``.
        """
        # Slice to the current sequence length: (1, sequence_length, d_model).
...
def attention(
    ...
    # (..., query_length, head_dim) @ (..., head_dim, key_length) -> (..., query_length, key_length)
    scores: Tensor = query @ key.transpose(-2, -1) / math.sqrt(head_dim)
    if mask is not None:
        # Mask broadcasts against (..., query_length, key_length); hidden positions get ~-inf.
        scores = scores.masked_fill(mask == 0, -1e9)
    # Normalize over key_length so each query row sums to one.
    probabilities: Tensor = nn.functional.softmax(scores, dim=-1)
    if dropout is not None:
        probabilities = dropout(probabilities)
    # (..., query_length, key_length) @ (..., key_length, head_dim) -> (..., query_length, head_dim)
...
class MultiHeadedAttention(nn.Module):
    ...
    def forward(
        ...
        Args:
            query: Queries shaped ``(batch_size, query_length, d_model)``.
            key: Keys shaped ``(batch_size, key_length, d_model)``.
            value: Values shaped ``(batch_size, key_length, d_model)``.
            mask: Optional mask shaped ``(batch_size, 1, key_length)`` or
                ``(batch_size, query_length, key_length)``.

        Returns:
            Attended states shaped ``(batch_size, query_length, d_model)``.
        """
        if mask is not None:
            # (batch_size, 1 | query_length, key_length) -> (batch_size, 1, 1 | query_length, key_length)
            # so the same mask broadcasts across every head.
            mask = mask.unsqueeze(1)
        batch_size: int = query.size(0)

        # (batch_size, sequence_length, d_model) -> view (batch_size, sequence_length, num_heads, head_dim)
        # -> transpose (batch_size, num_heads, sequence_length, head_dim)
        ...
        # attended: (batch_size, num_heads, query_length, head_dim)
        ...
        # (batch_size, num_heads, query_length, head_dim) -> (batch_size, query_length, num_heads * head_dim)
...
class Generator(nn.Module):
    ...
    def forward(self, inputs: Tensor) -> Tensor:
        ...
        Args:
            inputs: Decoder states shaped ``(batch_size, sequence_length, d_model)``.

        Returns:
            Log-probabilities shaped ``(batch_size, sequence_length, vocab)``.
        """
        # (batch_size, sequence_length, d_model) -> (batch_size, sequence_length, vocab)
...
class PositionwiseFeedForward(nn.Module):
    ...
    def forward(self, inputs: Tensor) -> Tensor:
        ...
        Args:
            inputs: States shaped ``(..., d_model)``.

        Returns:
            States shaped ``(..., d_model)`` after passing through ``d_ff``.
        """
...
def create_model(
    ...
    """
    shared_attention = MultiHeadedAttention(num_heads, d_model)
...
class LabelSmoothing(nn.Module):
    ...
    def forward(self, inputs: Tensor, target: Tensor) -> Tensor:
        ...
        Args:
            inputs: Log-probabilities shaped ``(num_tokens, vocab_size)``.
            target: Target token IDs shaped ``(num_tokens,)``.

        Returns:
            Scalar summed KL-divergence loss.

        Raises:
            ValueError: If the vocabulary dimension does not match ``size``.
        """
        # inputs: (num_tokens, vocab_size) log-probabilities; target: (num_tokens,) token IDs.
        if inputs.size(1) != self.size:
            raise ValueError("prediction vocabulary dimension does not match size")
        # Shape: (num_tokens, vocab_size); every entry starts at the smoothing mass.
        true_distribution: Tensor = inputs.detach().clone()
        true_distribution.fill_(self.smoothing / (self.size - 2))
        # Write ``confidence`` at each target ID using (num_tokens, 1) column indices.
        true_distribution.scatter_(1, target.detach().unsqueeze(1), self.confidence)
        true_distribution[:, self.padding_idx] = 0
        # Rows whose target is padding get an all-zero distribution.
        true_distribution[target == self.padding_idx] = 0.0
        self.true_dist = true_distribution
        return self.criterion(inputs, true_distribution)
...
class SimpleLossCompute:
    ...
    def __call__(self, x: Tensor, y: Tensor, norm: Tensor) -> Tensor:
        ...
        Args:
            x: Decoder states shaped ``(batch_size, sequence_length, d_model)``.
            y: Target token IDs shaped ``(batch_size, sequence_length)``.
            norm: Scalar count of non-padding tokens used to normalize the loss.

        Returns:
            Scalar total (un-normalized) loss for the batch.
        """
