...
# %% auto #0
__all__ = ['ENCODER_URL', 'VOCAB_URL', 'BPEPart', 'BPEWork', 'bytes_to_unicode', 'get_pairs', 'Encoder', 'get_file',
           'get_encoder', 'BPETokenizer']
...
class Encoder:
    ...
    def __init__(
        ...
        self.byte_decoder: dict[str, int] = {
            value: key for key, value in self.byte_encoder.items()
        }
        self.encoder = dict(encoder)
        self.decoder: dict[int, str] = {
            value: key for key, value in self.encoder.items()
        }
    ...
    def encode(self, text: str) -> list[int]:
        ...
        for token in self.pat.findall(text):
            translated: str = "".join(
                self.byte_encoder[byte] for byte in token.encode("utf-8")
            )
    ...
    def encode_and_show_work(self, text: str) -> BPEWork:
        ...
        for token in tokens:
            ...
            translated: str = "".join(
                self.byte_encoder[byte] for byte in token_bytes
            )
            merged_symbols: list[str] = self.bpe(translated).split(" ")
            part_ids: list[int] = [
                self.encoder[symbol] for symbol in merged_symbols
            ]
    ...
    def decode(self, bpe_idx: Sequence[int]) -> str:
        ...
        merged_tokens: list[str] = [
            self.decoder[token_id] for token_id in bpe_idx
        ]
...
def get_encoder(cache_dir: str | Path | None = None) -> Encoder:
    ...
    cache = (
        ...
        else Path.home() / ".cache" / "mingpt"
    )
    cache.mkdir(parents=True, exist_ok=True)

    encoder_file = cache / "encoder.json"
    get_file(encoder_file, ENCODER_URL)
    encoder_data: dict[str, int] = json.loads(
        encoder_file.read_text(encoding="utf-8")
    )
    ...
    bpe_merges: list[tuple[str, str]] = [
        ...
        if len(parts := merge_line.split()) == 2
    ]
