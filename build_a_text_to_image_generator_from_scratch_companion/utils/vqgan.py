...
VQGAN_CHECKPOINT_URL: str = "https://heibox.uni-heidelberg.de/f/140747ba53464f49b476/?dl=1"
VQGAN_CONFIG_URL: str = "https://heibox.uni-heidelberg.de/f/6ecf2af6c658432c8298/?dl=1"
...
def load_model(
    ...
    """
    checkpoint, config_file = download_vqgan_assets(checkpoint_path, config_path)
    ...
    checkpoint_data: dict[str, Any] = torch.load(checkpoint, map_location=device)
...
def process_image(file_name: str | Path, size: int = 384) -> Tensor:
    ...
    with Image.open(file_name) as source_image:
        ...
        # Shape: (channels, size, size) in [0, 1]
        image_tensor: Tensor = transform_functional.to_tensor(cropped)
    # (channels, size, size) -> (1, channels, size, size), rescaled to [-1, 1]
...
def image_to_sequence(
    ...
    Returns:
        Flattened codebook indices shaped ``(batch_size * latent_height * latent_width,)``,
        where each latent side is the input side divided by 16.
    """
    # f16 encoder: (batch_size, channels, height, width) -> (batch_size, z_channels, height / 16, width / 16)
    encoded: Tensor = model.encoder(image.to(device))
    # Shape: (batch_size, embed_dim, latent_height, latent_width)
    projected: Tensor = model.quant_conv(encoded)
    _, _, indices = model.quantize(projected)
    # Flattened codebook indices: (batch_size * latent_height * latent_width,)
    return indices[2]
...
def sequence_to_image(model: VQGANLike, sequence: Tensor) -> Tensor:
    ...
    Args:
        model: Loaded VQGAN model.
        sequence: Codebook indices shaped ``(576,)`` representing a 24-by-24 latent grid.

    Returns:
        Reconstructed image shaped ``(1, 3, 384, 384)`` in roughly ``[-1, 1]``.
    """
    # (num_tokens,) -> (num_tokens, embed_dim) = (576, 256) codebook vectors
    quantized: Tensor = model.quantize.embedding(sequence)
    # (num_tokens, embed_dim) -> (embed_dim, num_tokens) -> (1, embed_dim, latent_height, latent_width)
    quantized = quantized.permute(1, 0).view((1, 256, 24, 24))
    post_quantized: Tensor = model.post_quant_conv(quantized)
    # f16 decoder: (1, 256, 24, 24) -> (1, 3, 384, 384)
